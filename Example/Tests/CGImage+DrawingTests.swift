...
final class CGImage_DrawingTests: XCTestCase {
	...
	func drawTestImageInContext(context: CGContext, size: CGSize) {
		// blue background
		let backgroundColor = CGColorCreate(CGColorSpaceCreateDeviceRGB(), [0, 0, 1, 0.1])
	...
	func swiftLogo() -> CGImage {
		return try! CGImage.imageWithSize(CGSizeMake(256, 256)) { (context, size) in
	...
	func square() -> CGImage {
		return try! CGImage.imageWithSize(CGSizeMake(100, 100), flipped: false) { (context, size) in
			drawTestImageInContext(context, size: size)
		}
	}

	func testBlur() {
		let swiftLogo = self.swiftLogo()
		expect(swiftLogo).to(recordReferenceImageNamed("blur-pre"))
		let x = try! swiftLogo.imageByApplyingBlurWithRadius(5)
		expect(x).to(recordReferenceImageNamed("blur"))
